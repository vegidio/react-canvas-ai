...
export function useZoomPan(
    ...
    const getImageCoordinates = React.useCallback(
        ...
        [transform.scale, transform.translateX, transform.translateY, baseScale, contentSize, containerRef],
    );
    ...
    React.useEffect(() => {
        ...
        const resizeObserver = new ResizeObserver((entries) => {
            for (const entry of entries) {
                if (entry.target === container && contentSize.x > 0 && contentSize.y > 0) {
    ...
    }, [contentSize, containerRef, calculateBaseScale, onScaleChange, onPanChange]);
    ...
    const zoomToPoint = React.useCallback(
        (newScale: number, pointX: number, pointY: number) => {
            ...
            const rect = containerRef.current.getBoundingClientRect();
            const imagePointUnderCursor = getImageCoordinates(rect.left + pointX, rect.top + pointY);
            ...
            const finalOffsetX = (imagePointUnderCursor.x - contentSize.x / 2) * newCombinedScale - cursorOffsetX;
            const finalOffsetY = (imagePointUnderCursor.y - contentSize.y / 2) * newCombinedScale - cursorOffsetY;
        ...
        [baseScale, scale, contentSize, onScaleChange, onPanChange, getImageCoordinates],
    );
        ...
        (x: number, y: number) => {
            setTransform((prev) => {
                ...
                const shouldNotifyParent =
                    onPanChange && (prev.translateX !== constrainedX || prev.translateY !== constrainedY);
