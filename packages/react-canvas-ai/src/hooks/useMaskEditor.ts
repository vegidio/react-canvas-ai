...
export function useMaskEditor(props: UseMaskEditorProps): UseMaskEditorReturn {
    ...
    React.useEffect(() => {
        const listener = (evt: MouseEvent) => {
            ...
            const { x: imageX, y: imageY } = zoomPanActions.getImageCoordinates(evt.clientX, evt.clientY);
            ...
            if (maskContext && evt.buttons > 0 && !zoomPanState.isPanning && !zoomPanState.isSpaceKeyDown) {
        ...
        const scrollListener = (evt: WheelEvent) => {
            if (!evt.ctrlKey && !evt.metaKey && cursorContext && containerRef.current) {
                const { x: imageX, y: imageY } = zoomPanActions.getImageCoordinates(evt.clientX, evt.clientY);
    ...
    React.useEffect(() => {
        ...
        if (initialMaskAppliedRef.current === initialMask || initialMaskApplyingRef.current) return;
            ...
            const attemptDraw = () => {
                ...
                try {
                    ...
                    try {
                        ...
                        console.error('[MaskEditor] Error drawing image on canvas:', drawError);
                    ...
                    console.error('[MaskEditor] Error getting canvas context:', contextError);
                }
            };
        ...
        const loadImage = async () => {
            ...
            img.onerror = () => {
                ...
                if (src.startsWith('http')) {
                    ...
                    console.log("[MaskEditor] This appears to be a data URL. Verify it's not corrupted or too large.");
                } else {
                    console.log(
            ...
            try {
                ...
                console.error('[MaskEditor] Unexpected error during image load initialization:', error);
    ...
    const handleMouseDown = React.useCallback(
        (e: React.MouseEvent<HTMLCanvasElement>) => {
            ...
            if (maskContext) {
                maskContext.beginPath();
                maskContext.fillStyle = e.buttons > 1 || e.shiftKey ? '#ffffff' : maskColor;
